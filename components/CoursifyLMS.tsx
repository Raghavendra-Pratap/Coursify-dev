...
const CoursifyLMS = () => {
  ...
  const fallback = undefinedPages.length > 0 ? (
    <div className="p-8 text-center text-gray-600">
      <p className="text-red-600">Missing component(s): {undefinedPages.join(', ')}. Check exports in components/pages.</p>
    </div>
  ) : null;
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Sign in to Coursify</h1>
          <p className="text-gray-600 mb-6">You need to sign in to access the dashboard and courses.</p>
          <form onSubmit={handleSignIn} className="space-y-4">
            {signInError && (
              <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">{signInError}</div>
            )}
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Email</label>
              <input type="email" value={signInEmail} onChange={(e) => setSignInEmail(e.target.value)} placeholder="you@example.com" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Password</label>
              <input type="password" value={signInPassword} onChange={(e) => setSignInPassword(e.target.value)} placeholder="••••••••" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required />
            </div>
            ...
            <button type="button" onClick={handleSignInWithGoogle} className="w-full py-3 border border-gray-300 rounded-xl hover:bg-gray-50 font-semibold flex items-center justify-center gap-2">
  ...
  return (
    <AppLayout>
      ...
      {/* Sign In Modal */}
      {showSignInModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => { setShowSignInModal(false); setSignInError(null); }}>
            ...
            <form onSubmit={handleSignIn} className="p-6">
              ...
              <div className="mt-6 space-y-3">
                ...
                <button
                  ...
                  className="w-full py-3 border border-gray-300 rounded-xl hover:bg-gray-50 font-semibold transition-all flex items-center justify-center gap-2"
                >
                  <svg className="w-5 h-5" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                  Sign in with Google
                </button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6">
              ...
              <div className="text-center mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-900">{userDisplay.displayName}</h2>
                <p className="text-gray-600 mt-1">{userDisplay.role}</p>
                {userDisplay.email && <p className="text-sm text-gray-500 mt-1">{userDisplay.email}</p>}
              </div>
              {userDisplay.role === 'Sign in to save' && (
                <div className="mb-6">
                  <button
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="bg-slate-100 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold text-blue-600">{profileStats.courses}</p>
                  <p className="text-sm text-gray-600 mt-1">Courses</p>
                </div>
                <div className="bg-green-50 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold text-green-600">{profileStats.certificates}</p>
                  <p className="text-sm text-gray-600 mt-1">Certificates</p>
                </div>
                <div className="bg-purple-50 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold text-purple-600">{profileStats.badges}</p>
              ...
              <div className="space-y-2">
                <button 
                  ...
                  className="w-full px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-xl flex items-center justify-between transition-all text-left"
                >
                  <span className="flex items-center font-semibold text-sm text-gray-800">
                ...
                <button 
                  ...
                  className="w-full px-4 py-3 bg-red-50 hover:bg-red-100 rounded-lg flex items-center justify-between transition-all text-red-600"
                >
